...
func NewGeneratorFactory(config *FactoryConfig) *GeneratorFactory {
	...
	config.TemplateRoot = resolveTemplateRoot(config.TemplateRoot)
...
func resolveTemplateRoot(root string) string {
	...
	return filepath.Clean(filepath.Join(getProjectRoot(), root))
}
...
func getProjectRoot() string {
	rootOnce.Do(func() {
		...
		projectRootPath = filepath.Clean(filepath.Join(dir, "..", "..", ".."))
	})
...
func (f *GeneratorFactory) ValidateRequest(req *GenerateRequest) error {
	...
	if stackConfig, exists := f.config.StackConfigs[stack]; exists {
		for _, feature := range req.Features {
			...
			if !validFeature {
				return fmt.Errorf("invalid feature '%s' for stack '%s'", feature, stack)
			}
		}
	}
