Perfeito.
Seguindo **exatamente o formato do arquivo `mcp-core-inventory-INTEGRACOES-v2.md`** (colunas: **ORIGEM | INTEGRA | MOTIVO**), aqui está a **tabela oficial de integrações do BLOCO-1**.
...
* consolidar tudo isso em um **único `mcp-core-inventory-INTEGRACOES.md` v3**,
...
Baseado nos arquivos `mcp-core-inventory-INTEGRACOES.md` e `mcp-core-inventory-ARVORE-FULL.md`, preparei três visualizações estratégicas:
...
Você pode incluir estes blocos Mermaid diretamente no seu arquivo `mcp-core-inventory-INTEGRACOES.md` (se o seu visualizador Markdown suportar) ou na documentação `docs/architecture/blueprint.md` citada na árvore de arquivos. Eles servem como a "prova visual" de que a arquitetura modular monolítica do Hulk é coesa.
